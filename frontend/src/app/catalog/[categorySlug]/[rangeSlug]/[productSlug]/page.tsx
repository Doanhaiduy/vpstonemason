...
export default function ProductDetailPage({ params }: PageProps) {
  const { categorySlug, rangeSlug, productSlug } = params;
  ...
  const fetchData = useCallback(async () => {
    ...
    setError(false);
    try {
      const result = await api.getCatalogItem(productSlug);
      setData(result);
    } catch {
      setError(true);
    } finally {
  ...
  useEffect(() => {
    ...
    setError(false);
    setLoading(false);
  }, [categorySlug, fetchData, isPreview, productSlug, rangeSlug]);
  ...
  useEffect(() => {
    if (subGalleryIndex >= verticalGalleryImages.length) {
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
            <div>
              {galleryImages.length > 0 ? (
                <>
                  <motion.div
                    className="aspect-square bg-stone-100 overflow-hidden mb-4 cursor-pointer"
                    ...
                    <div className="grid grid-cols-4 gap-3">
                      {thumbnailImages.map((image, index) => (
                        <button
                          ...
                          className={`aspect-square bg-cover bg-center border-2 transition-all ${
                            index === activeImage
                              ? 'border-stone-900'
              ...
              <div className="relative">
                ...
                >
                  <motion.div
                    ...
                    style={{
                      backgroundImage: `url('${verticalGalleryImages[subGalleryIndex]}')`,
                    }}
                  />
                </button>

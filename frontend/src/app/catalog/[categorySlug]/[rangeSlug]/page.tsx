...
export default function RangeDetailPage({ params }: PageProps) {
  const { categorySlug, rangeSlug } = params;
  ...
  const fetchData = useCallback(async () => {
    ...
    setError(false);
    try {
      const result = await api.getCatalogItem(rangeSlug);
      setData(result);
    } catch {
      setError(true);
    } finally {
  ...
  useEffect(() => {
    ...
    setError(false);
    setLoading(false);
  }, [categorySlug, fetchData, isPreview, rangeSlug]);
          ...
          <div className="container-custom py-12 md:py-16">
            ...
            >
              <div
                ...
                style={{ backgroundImage: `url('${item.imageMain}')` }}
              />
            </motion.div>
          </div>
        </section>
